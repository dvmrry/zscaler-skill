...
This file is the skill's ad-hoc front door. It is invoked via `@zscaler` (Claude Code: `@zsca<tab>`; Windsurf/Cascade: `@file:zscaler<tab>`) — there is no slash command, because there is no procedural harness to drive. The output shape is a conversational answer with citations, not a structured artifact.
