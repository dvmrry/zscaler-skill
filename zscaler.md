...
This file is the skill's ad-hoc front door. It is invoked via `@zscaler.md` followed by a question (tab-completion availability varies by runtime — Cascade currently requires the full filename). There is no slash command, because there is no procedural harness to drive. The output shape is a conversational answer with citations, not a structured artifact.
