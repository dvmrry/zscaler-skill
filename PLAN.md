...
6. **ZIdentity deeper** (Section B) — Conditional Access mechanics now cross-referenced from `shared/cross-product-integrations.md`; a dedicated ZIdentity doc would let the skill reason about auth-flow internals (SAML / OIDC / step-up-auth lifecycle, IdP integration patterns, Conditional Access rules themselves), not just cite the feature. Next candidate.
