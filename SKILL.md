...
**For "would this URL be blocked?" questions** — use the policy simulator at [`references/_meta/policy-simulation.md`](references/_meta/policy-simulation.md). A pure-function ZIA URL filter evaluator that takes a snapshot + request and returns the matched rule plus an evaluation trace. Source at `scripts/policy_simulator.py`; runnable CLI at `scripts/simulate-policy.py`. Models rule order, disabled-rule-holds-slot, basic category resolution, and criteria filtering. Documented gaps: CAC, DLP, two-pass SSL inspection, full specificity-wins-across-custom-categories pattern matching. Use for first-pass single-URL intuition; verify against logs once available. (PR-level change-validation harness deferred pending log access for corpus quality + CI plan-artifact ingestion.)
