...
title: "Investigation diagnostics — template + private-fork pattern"
...
# Investigation diagnostics — template

A **diagnostic** is a named, ordered sequence of queries (API calls, SPL patterns, manual tests) that the `/z-investigator` agent runs against a specific hypothesis to confirm or rule it out. Diagnostics encode the steps an experienced analyst would take, with decision logic mapping results to journal claim statuses.

## Why this file ships empty

Diagnostics are only useful when they're **verified**. Wrong queries, wrong thresholds, or wrong decision logic misleads investigations in ways that are hard to catch downstream. A diagnostic invented without ground truth — without a real ticket worked through, a lab tenant reproduction, or a Zscaler vendor doc prescribing the exact sequence — produces plausible-looking content that can be subtly wrong.
...
| Location | Use when |
|---|---|
| **Private fork** of this skill | Diagnostics specific to your tenant / SIEM plumbing / internal ticketing |
| **CLAUDE.md** (project or user level) | A short diagnostic the agent can pick up automatically every session |
| **`_local-diagnostics/`** (gitignored directory) | Larger diagnostic collections kept alongside the skill but not committed |
| **PR back to this repo** | A diagnostic that generalizes — placeholder plumbing only, queries derived from Zscaler-published surfaces, verification cited |
...
`_local-diagnostics/sipa-port-22.md`" OR "Lab tenant 'sandbox-a' on
...
- Zscaler-Support-enabled fields the diagnostic relies on
  (e.g., `clt_sport`, `srv_dport`, `dlprulename`)
- Conditions where this diagnostic's logic is wrong
...
Map to a single hypothesis from the methodology's prioritization tiers (configuration / connector health / destination / policy edge case). One diagnostic = one hypothesis. If a diagnostic would cover multiple hypotheses, split it — keeps decision logic clear.

### Verification (the gating field)

A diagnostic without verification is speculation. Acceptable verification:

- Real production ticket worked through end-to-end with the diagnostic as documented
...
If you cannot cite verification, the diagnostic is not ready to ship.
...
If the diagnostic's correctness depends on:
...
Document it explicitly. Silent dependencies cause confused investigations when the diagnostic is run in environments that don't satisfy them.
...
- Placeholder plumbing in any diagnostic published outside your private fork
- Tenant identifiers (real index / sourcetype / user / hostname / ticket number) stay in private fork or local-only diagnostics
...
- [`investigator/prompt.md`](../prompt.md) — `/z-investigator` playbook
...
- [`architect/diagnostics/template.md`](../../architect/diagnostics/template.md) — parallel template for `/z-architect` recommendation diagnostics
