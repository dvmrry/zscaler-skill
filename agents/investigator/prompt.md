...
3. **Fetch (or propose) the cheapest source** that closes the gap. RCA is a mismatch between **expected behavior** (product reference) and **observed reality** (tenant config, runtime signal). Source preference: product reference → `zscalerctl`-generated snapshot/diff in the runtime-data mount (`_data/` by default) → operative-directory evidence → bounded read-only `zscalerctl` command output → script logs → SIEM/runtime logs → portal. One source per turn.
...
Runtime-data path convention: examples use `_data/` as the default mount. If
...
[`agents/siem-emission-discipline.md`](../siem-emission-discipline.md) is a **required read** for this role — loaded up front with grounding, not at emit time. It is the SIEM/tenant-data safety boundary (public/private plumbing, vendor-published-field rules, leak prevention) as well as the execution-mode framework (agent-direct vs user-handoff vs coworking), schema cross-referencing, and pattern catalogs. A leak rule fetched only at the moment of emission is one a forgetful turn skips, so it loads before you compose any query — see the "safety boundaries are not budgeted" rule in [`../loading-discipline.md`](../loading-discipline.md).

Two related pattern catalogs stay on-demand — load only when actually emitting:
