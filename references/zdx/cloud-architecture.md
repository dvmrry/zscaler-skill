...
API access has a dedicated legacy credential stack (API key_id + key_secret, SHA256-signed) that is separate from ZIdentity/OneAPI — but ZDX is **also** OneAPI-reachable (the Go SDK detects ZDX paths and routes them through its ZDX OneAPI HTTP client, `vendor/zscaler-sdk-go/zscaler/oneapiclient.go:376-377,396-397`). See [`./api.md § Auth`](./api.md).
...
ZDX has a dedicated legacy auth flow: OAuth 2.0 with API key_id + key_secret credentials from the ZDX Admin Portal (this key flow is **not** ZIdentity/OneAPI). ZDX is *also* OneAPI-capable — the Go SDK detects ZDX paths and routes them through its ZDX OneAPI HTTP client (`vendor/zscaler-sdk-go/zscaler/oneapiclient.go:376-377,396-397`); see [`./api.md § Auth`](./api.md). (`vendor/zscaler-help/legacy-api-authentication-zdx.md:8-27`)
...
**Relationship to ZIdentity/OneAPI:** The Getting Started guide distinguishes the ZDX *legacy key* credential system from OneAPI: "If you need to obtain API keys, authenticate into, and make calls using Zscaler OneAPI endpoints, see About API Clients in ZIdentity..." — i.e., the ZDX key_id/key_secret flow is a separate credential system from OneAPI. It does **not** follow that ZDX is OneAPI-absent: the Go SDK detects ZDX paths and routes them through its ZDX OneAPI HTTP client (`vendor/zscaler-sdk-go/zscaler/oneapiclient.go:376-377,396-397`), so the legacy key flow coexists with OneAPI rather than replacing it. (`vendor/zscaler-help/legacy-getting-started-zdx-api.md:17`)
