...
sources:
  ...
  - "vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/list_applications.py"
...
### MCP v0.15.0 record and timestamp behavior
...
The MCP `StartDeepTraceInput` workflow directs callers to resolve `app_id` with `zdx_list_applications`, `web_probe_id` with `zdx_get_web_probes(device_id, app_id)`, and `cloudpath_probe_id` with `zdx_list_cloudpath_probes(device_id, app_id)` before starting a deeptrace (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/deeptrace_manage.py:29-35`, `:103-110`). Its input schema declares all three ID fields as required integers (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/deeptrace_manage.py:37-48`). This is the MCP tool's Pydantic surface, not the API contract — the Python SDK docstring types `app_id` / `web_probe_id` / `cloudpath_probe_id` as `str` (`vendor/zscaler-sdk-python/zscaler/zdx/troubleshooting.py:149`, `:153`, `:154`); the current MCP start path coerces the three payload IDs to integers before calling the SDK (see the divergences section below).
...
Source: `vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/_common.py`; `vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/device_web_probes.py`; `vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/device_cloudpath_probes.py`; `vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/list_applications.py`; `vendor/zscaler-sdk-python/zscaler/utils.py`; `vendor/zscaler-sdk-python/zscaler/zdx/devices.py`; `vendor/zscaler-sdk-python/zscaler/zdx/apps.py`.

The shared probe input declares `since` as an optional integer number of hours with a Pydantic default of `None`; its description says ZDX defaults the look-back to 2 hours when omitted (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/_common.py:195-203`). The web- and cloud-path-probe tools forward `since` only when it is supplied (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/device_web_probes.py:35-41`; `vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/device_cloudpath_probes.py:35-43`). `ListApplicationsInput.since` has the same optional-integer hours shape and documented ZDX default, and the application tool forwards it through `scope_query_params` (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/list_applications.py:25-40`, `:97-104`).
...
Source: `vendor/zscaler-sdk-python/zscaler/zdx/troubleshooting.py`; `vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/deeptrace_manage.py`; `vendor/zscaler-sdk-go/zscaler/zdx/services/troubleshooting/deeptrace/deeptrace.go`.

`start_deeptrace` is `POST /zdx/v1/devices/{device_id}/deeptraces`. `device_id` is a path segment; the three required IDs go in the JSON body, which is the kwargs dict passed straight through (`session_name`, `app_id`, `web_probe_id`, `cloudpath_probe_id`, `session_length_minutes`, `probe_device`) (`vendor/zscaler-sdk-python/zscaler/zdx/troubleshooting.py:176-182`; MCP `sdk_kwargs` assembled and sent at `vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/deeptrace_manage.py:113-121`).

`session_length_minutes` defaults to 5 and is documented as supporting **5, 15, 30, 60** minutes by the MCP input description (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/deeptrace_manage.py:49-52`) and the Python SDK docstring (`vendor/zscaler-sdk-python/zscaler/zdx/troubleshooting.py:155-156`). The current MCP Pydantic field does not runtime-constrain the value to that set; it documents the supported durations.

`delete_deeptrace` is a separate destructive operation (not part of the prerequisite chain): `DELETE /zdx/v1/devices/{device_id}/deeptraces/{trace_id}` (`vendor/zscaler-sdk-python/zscaler/zdx/troubleshooting.py:223-227`); the MCP delete input declares both `device_id` and `trace_id`, and the tool rejects a request missing either one (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/deeptrace_manage.py:59-63`, `:141-149`).

### Python-vs-Go SDK divergences on the deeptrace path

Source: `vendor/zscaler-sdk-go/zscaler/zdx/services/troubleshooting/deeptrace/deeptrace.go`; `vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/deeptrace_manage.py`; `vendor/zscaler-sdk-python/zscaler/zdx/troubleshooting.py`; `vendor/zscaler-sdk-go/zscaler/zdx/services/common/common.go`; `vendor/zscaler-sdk-python/zscaler/utils.py`.

- **Cloudpath probe wire key (highest-value divergence for anyone hand-building the body):** Go serializes the cloudpath probe ID as JSON key `cloud_path_probe_id` (with an underscore between `cloud` and `path`) — `DeepTraceSessionPayload.CloudPathProbeID` has tag `json:"cloud_path_probe_id"` (`vendor/zscaler-sdk-go/zscaler/zdx/services/troubleshooting/deeptrace/deeptrace.go:45`). The Python path sends `cloudpath_probe_id` (no underscore) (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/deeptrace_manage.py:113-121`, especially `:117`; `vendor/zscaler-sdk-python/zscaler/zdx/troubleshooting.py:182`).
- **Payload ID types agree (both int):** Go types `AppID` / `WebProbeID` / `CloudPathProbeID` / `SessionLengthMinutes` as `int` (`vendor/zscaler-sdk-go/zscaler/zdx/services/troubleshooting/deeptrace/deeptrace.go:41-48`); the Python MCP tool coerces `app_id` / `web_probe_id` / `cloudpath_probe_id` to `int(...)` before sending (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/deeptrace_manage.py:115-117`). IDs are integers on the wire for the start payload in both.
...
## MCP v0.15.0 guided-prompt drift
...
- ~~What the documented session duration set is~~ — **resolved** from SDK and MCP documentation: `session_length_minutes` defaults to 5 and supports 5, 15, 30, or 60 minutes (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/deeptrace_manage.py:49-52`; `vendor/zscaler-sdk-python/zscaler/zdx/troubleshooting.py:155-156`). The current MCP field documents but does not runtime-constrain that set. See the start-payload section above.
...
- **Maximum `since` / look-back window and probe-ID expiry (unverified).** The shared probe input declares `since` as `Optional[int]` with a Pydantic default of `None`, while its description says ZDX defaults the look-back to 2 hours (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/_common.py:195-203`). The probe tools pass it only when supplied (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/device_web_probes.py:35-41`; `vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zdx/device_cloudpath_probes.py:35-43`). No cited source states a maximum look-back window or whether probe IDs expire after that window. See [clarification zdx-34](../_meta/clarifications.md#zdx-34-maximum-look-back-window-and-probe-id-expiry).
