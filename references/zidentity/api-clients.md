...
- **Revocation, disable, and rotation timing — the Access-Tokens help pages.** The `About Access Tokens` and `Revoking Access Tokens` articles are named in the related-articles list of vendor/zscaler-help/zidentity-about-api-clients.md but are not captured in `vendor/zscaler-help/`. The "revocation takes effect at the next OneAPI call" timing and post-revocation propagation window, **and the disabled-client token-survival window and secret-rotation grace period stated in Common misconfiguration patterns #4–#5**, are therefore unverified against source — those specific timings are inferred from standard OAuth 2.0 bearer-token semantics. Capture those pages or confirm against tenant behavior. — see [clarification `zid-33`](../_meta/clarifications.md#zid-33-about-revoking-access-tokens-articles-uncaptured)
