...
- **Secrets snapshot file layout** — if api-client secrets are dumped, the per-client file/key naming convention (e.g. `api-client-secrets.json` keyed by client id) is undefined here; the `value` field makes that output sensitive and may warrant exclusion from the snapshot entirely. — see [clarification `zid-27`](../_meta/clarifications.md#zid-27-secrets-snapshot-file-layout)
