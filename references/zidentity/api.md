...
ZIdentity API base path differs between the two SDKs against the **same host** (`https://api.zsapi.net`, `vendor/zscaler-sdk-python/zscaler/request_executor.py:32` BASE_URL; `vendor/zscaler-sdk-go/zscaler/oneapiclient.go:416` `GetAPIBaseURL`):
...
The request URLs genuinely differ: Python POSTs `https://api.zsapi.net/ziam/admin/api/v1/users` while Go POSTs `https://api.zsapi.net/admin/api/v1/users`. The `/admin` prefix is also how the Go gateway router classifies these calls — `detectServiceType` maps a `/admin` path prefix to the `"admin"` (default, non-ZIA/ZPA) service type (`vendor/zscaler-sdk-go/zscaler/oneapiclient.go:409`). If you write a raw-HTTP caller, match the prefix to whichever SDK's convention you are following; both reach the same gateway. The endpoint tables in §1 and §4 below use the Python `/ziam`-prefixed form — strip the `/ziam` prefix for the Go equivalents.
