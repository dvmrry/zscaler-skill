...
- **`scan_type` allowed values.** Present as an attribute (`vendor/zscaler-sdk-python/zscaler/zeasm/models/findings.py:84`) but the source gives no enumeration, validator, or docstring describing its possible values. Allowed values unverified from source.
