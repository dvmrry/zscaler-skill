...
| ~~`about-machine-groups.md`~~ | ~~ZIA machine groups~~ → **mis-classified; this is a ZPA construct** | Vendor URL is `help.zscaler.com/zpa/about-machine-groups`; SDK lives at `zscaler/zpa/machine_groups.py` and `terraform-provider-zpa`; no Machine Groups entity in ZIA. Covered in `references/zpa/machine-groups.md`. | — (resolved) |
