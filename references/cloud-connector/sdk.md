...
Source: `vendor/zscaler-sdk-go/zscaler/oneapiclient.go:207-208,372-373,390-391`; `vendor/zscaler-sdk-python/zscaler/oneapi_client.py:30,104-108,288-294`.
...
- Go: the unified client routes ZTW to a dedicated OAuth2 HTTP client. `getServiceHTTPClient` returns `client.oauth2Credentials.ZTWHTTPClient` for the `ztw` service (`oneapiclient.go:372-373`), and `detectServiceType` classifies any endpoint whose path is prefixed `ztw` as the ZTW service (`oneapiclient.go:390-391`). ZTW also has its own rate limiter — "ZTW uses same limits as ZIA" — wired as `ztwRateLimiter` (`oneapiclient.go:207-208`).
...
OneAPI uses Zscaler OAuth 2.0 Client Credentials via ZIdentity. **FedRAMP availability is unresolved across current sources.** The shared Go client constructs dedicated `gov` / `govus` identity and API endpoints, and the ZTC provider passes its unrestricted `zscaler_cloud` value into that client (`vendor/zscaler-sdk-go/zscaler/oneapiclient.go:404-438`; `vendor/terraform-provider-ztc/ztc/provider.go:44-49`; `vendor/terraform-provider-ztc/ztc/config.go:350-372`). In tension with that route support, Terraform Skills v0.3.1 says no released ZTC version supports OneAPI FedRAMP, while the provider docs claim support only at `>=v4.7.25`—an impossible threshold for the current v0.2.0 ZTC provider (`vendor/zscaler-terraform-skills/skills/ztc-skill/references/auth-and-providers.md:5-16`; `vendor/terraform-provider-ztc/docs/index.md:143-152`; `vendor/terraform-provider-ztc/CHANGELOG.md:3-12`). Route construction is not evidence that a live ZTW tenant accepts the operation. Retain legacy auth for a production government tenant unless current release notes, vendor confirmation, or a tenant test establishes OneAPI support; see [clarification `cloud-connector-19`](../_meta/clarifications.md#cloud-connector-19-ztw-sdk-method-convention-anomalies-and-oneapi-fedramp-behavior).
...
| `ForceActivationStatus` | `(ctx, service, forceActivation ECAdminActivation) (*ECAdminActivation, error)` | `PUT /ztw/api/v1/ecAdminActivateStatus/forcedActivate` | Force-activates; static sources do not document lock, validation-bypass, or recovery semantics |
...
2. **`provisioning_url` uses non-`Resource` methods**: `Create`, `UpdateWithPut`, and `Delete` in `provisioning_url` use the ZIA-style methods (`service.Client.Create`, not `CreateResource`). Whether this is intentional or a bug is not confirmed from available sources. Filed with the `workload_groups.Get` anomaly and OneAPI FedRAMP behavior as [clarification `cloud-connector-19`](../_meta/clarifications.md#cloud-connector-19-ztw-sdk-method-convention-anomalies-and-oneapi-fedramp-behavior).
...
7. **Resolved 2026-04-26.** `workload_groups.Get` calls `service.Client.Read` (not `ReadResource`) — confirmed in `vendor/zscaler-sdk-go/zscaler/ztw/services/workload_groups/workload_groups.go`. This is inconsistent with the ZTW convention of using `ReadResource` for GET operations. The workload groups endpoint may use the ZIA-compatible request path rather than the ZTW Resource-suffixed path; the exact reason is not documented in available sources. Residual (why the convention differs) filed as [clarification `cloud-connector-19`](../_meta/clarifications.md#cloud-connector-19-ztw-sdk-method-convention-anomalies-and-oneapi-fedramp-behavior).

8. **Reopened 2026-08-09.** OneAPI **is** a first-class ZTW service in both SDKs (Go `oneapiclient.go:372-373,390-391,207-208`; Python `oneapi_client.py:30,104-108,288-294`), with the legacy username/password/API-key path retained as the alternative. The earlier claim that OneAPI is categorically excluded for `zscalergov` / `zscalerten` is no longer supportable: current SDK code builds `gov` / `govus` routes, current provider docs claim support with an invalid copied version floor, and Terraform Skills says the released ZTC path remains legacy-only. The open question is released/live FedRAMP acceptance, not fallback behavior after a confirmed exclusion; see [clarification `cloud-connector-19`](../_meta/clarifications.md#cloud-connector-19-ztw-sdk-method-convention-anomalies-and-oneapi-fedramp-behavior).
