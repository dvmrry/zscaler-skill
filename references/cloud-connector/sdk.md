...
Workload groups (tag-based workload abstractions used in forwarding rule `src_workload_groups`). Create/Update/Delete are exported in the SDK source. The ZTC Terraform provider exposes a native `ztc_workload_groups` **data source** (`vendor/terraform-provider-ztc/ztc/provider.go:146`) for read/ID lookup, so `src_workload_groups` IDs can be resolved from the ZTC provider directly — they no longer require the ZIA provider's `zia_workload_groups` data source. There is still no ZTC workload-groups **resource** (mutation must go through the SDK/API Create/Update/Delete, or be authored ZIA-side).
