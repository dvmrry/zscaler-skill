...
| `workload_groups` | `/workloadGroups` | Workload group CRUD — tag-based workload abstractions for policy. (`workload_groups/workload_groups.go:13`) |
...
- **Whether the legacy `ZTC_*` credential surface is the only Go ZTW auth path** — the vendored Go ZTW config exposes only the legacy CBC/ZTC credentials (see § Go-SDK authentication status); whether a ZIdentity OAuth path exists for Go ZTW isn't confirmable from this capture. See [clarification `cloud-connector-18`](../_meta/clarifications.md#cloud-connector-18-ztw-api-surface-gaps-endpoint-paths-azuregcp-discovery-automation-go-zidentity-auth).
