...
- **`ip_pool_groups`:** the Terraform resource `ztc_ip_pool_groups` (+ data source) is **backed by the Go SDK** — `resource_ztc_ip_pool_groups.go:13` imports `ztw/services/policyresources/ipgroups` and calls its full CRUD (`ipgroups.Create`/`Get`/`Update`/`Delete`, `ipgroups.go`). So the functionality IS in the Go SDK; there is simply no package *named* `ip_pool_groups` — it maps onto the differently-named `policyresources/ipgroups`. The Python `ztw` package has no `ip_pool_groups` accessor (confirmed absent: it has `ip_groups.py`/`ip_source_groups.py`/`ip_destination_groups.py` only).
