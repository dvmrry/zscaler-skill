...
Gotchas:
- `src_workload_groups` IDs: source disagrees with itself, so use the ZIA fallback and treat the native data source as unconfirmed. A native `ztc_workload_groups` data source is registered in code (`vendor/terraform-provider-ztc/ztc/provider.go:146`, backed by `data_source_ztc_workload_groups.go` — schema fields `id`/`name`/`description`/`expression`/`expression_json` at `:17`,`:23`,`:28`,`:33`,`:38`), but the vendor's own resource docs still direct practitioners to the ZIA Terraform provider's `zia_workload_groups` data source: `vendor/terraform-provider-ztc/docs/resources/ztc_traffic_forwarding_rule.md:52-53` states verbatim "To retrieve the src_workload_groups ID information, you must leverage the ZIA Terraform Provider at the moment, which returns the exact same information ID since the resource is cross-shared between ZIA and ZTC." All three HCL examples on that page use `data "zia_workload_groups"` accordingly (`:55-57`, `:131-133`, `:195-197`). Whether the registered-in-code `ztc_workload_groups` data source is production-ready is not confirmed from source alone — the code registration and the resource docs disagree. There is still no `ztc_workload_groups` *resource* for mutation — create/update/delete go through the SDK/API or are authored ZIA-side.
