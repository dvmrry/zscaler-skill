...
11. **Rule-label support on `ztc_traffic_forwarding_rule`**: The provider schema has no `labels` / `rule_label` attribute, but the reconstructed ZCloudConnector contract models a `labels` array for `/ecRules/ecRdr`. Whether the API accepts label writes for traffic forwarding rules, and whether the Terraform provider will add support, is unresolved. See [clarification `cloud-connector-26`](../_meta/clarifications.md#cloud-connector-26-ztc_traffic_forwarding_rule-label-support).
