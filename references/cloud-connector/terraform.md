...
2. **Resolved 2026-04-26.** `ztc_traffic_forwarding_log_rule` doc label is a confirmed bug. The H1 header reads `# ztc_traffic_forwarding_log_rule (Data Source)` but the file is under `docs/resources/` and the resource is implemented as a managed resource. The `(Data Source)` label in the header is incorrect.
