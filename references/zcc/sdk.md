...
1. `update_zdx_group_entitlement` and `update_zpa_group_entitlement` both send an empty body (`{}`). The intended use is not clear from the Python source — there may be required fields missing from the implementation, or the endpoint may rely entirely on URL-derived context. Requires API documentation review.
...
3. The `web_policy_edit` method calls `transform_common_id_fields(reformat_params, body, body)` but does not document which fields this affects. The behavior of `reformat_params` needs tracing in `zscaler/utils.py` to enumerate what ID transformations are applied.

4. Application profiles, custom IP apps, predefined IP apps, and process-based apps exist in the Go SDK but have no Python equivalent. Whether these are intentionally excluded from the Python SDK or simply not yet implemented is unknown.

5. The `manage_pass` endpoint in Go (`managePass`) accepts per-product passwords (ZIA, ZPA, ZDX, exit, logout, uninstall). Python SDK has a `models/manage_pass.py` model file but no corresponding service method — the service layer is absent.
