...
For ZCC list methods that expose pagination, callers pass `page` and `page_size` in `query_params`. The `list_devices` docstring identifies a default page size of 50 and a maximum of 5000 (`vendor/zscaler-sdk-python/zscaler/zcc/devices.py:290-308`); the retained `getDevices` Automate contract independently documents `page`/`pageSize`, the same default of 50, and the same maximum of 5000 (`vendor/zscaler-api-specs/automate-zscaler/zcc-api-reference.json:6963-6992`). The parameter conversion sends the Python `page_size` spelling as the API's `pageSize` key (`vendor/zscaler-sdk-python/zscaler/request_executor.py:504-509`).
