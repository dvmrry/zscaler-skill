...
The modern OneAPI client path uses the shared `RequestExecutor` from the Python SDK, and it **does** honor the ZCC rate-limit header when that header is present. `get_retry_after()` reads `X-Rate-Limit-Retry-After-Seconds` (falling back to `X-Rate-Limit-Remaining`) under an inline comment that names these as "ZCC Specific Rate Limiting Headers (LegacyZCCClientHelper)" (`vendor/zscaler-sdk-python/zscaler/request_executor.py:943-951`). The retry loop treats 429 as retryable — `is_retryable_status()` includes `HTTPStatus.TOO_MANY_REQUESTS` (`vendor/zscaler-sdk-python/zscaler/request_executor.py:771-784`) — and on a retryable status it calls `get_retry_after()` and sleeps that long before retrying (`vendor/zscaler-sdk-python/zscaler/request_executor.py:755-766`).

Retry budget: the default is `maxRetries = 2`, read from `config["client"]["rateLimit"]` (`vendor/zscaler-sdk-python/zscaler/request_executor.py:83-86`). The computed backoff has a +1s pad applied to either recognized header path (`vendor/zscaler-sdk-python/zscaler/request_executor.py:957-966`), and an optional `rateLimit.maxRetrySeconds` cap can short-circuit waits judged too long (`vendor/zscaler-sdk-python/zscaler/request_executor.py:974-984`).
