...
Plus these flags that look boolean-ish but are `int` (0/1) on the wire: `enableLWFDriver` (string in Go, uniquely), `enableSplitVpnTN` (line 39), `enableUnifiedTunnel` (line 36), `enableAllDefaultAdaptersTN` (line 38), `evaluateTrustedNetwork` (line 40), `skipTrustedCriteriaMatch` (line 41), `systemProxy`, `enablePacketTunnel`, `allowTLSFallback`, `pathMtuDiscovery`, `optimiseForUnstableConnections`, `useTunnel2ForProxiedWebTraffic`, `useTunnel2ForUnencryptedWebTraffic` (line 67), `sendAllDNSToTrustedServer`.
...
| `condition_type` | `conditionType` | How trusted-criteria combine (AND vs OR across the inline fields + referenced TrustedNetworks). Typed `int` (`forwarding_profile.go:22`); enum values not documented in SDK — see [`clarification zcc-01`](../_meta/clarifications.md#zcc-01-forwardingprofile-condition_type-enum). |
...
| `tunnel2_fallback_type` | `tunnel2FallbackType` | Specific fallback behavior when Z-Tunnel 2.0 fails. Typed `int` (`forwarding_profile.go:61`). Python SDK inconsistency: config branch uses `tunnel2_fallback` (line 172) but fallback branch uses `tunnel2_fallback_type` (line 219) — attribute name mismatch in the Python model. |
...
**Captive portal settings were migrated from tenant-global to App Profile scope** per the ZCC release notes and the *About Zscaler Client Connector App Profiles* help article (`about-zscaler-client-connector-app-profiles.md:17`). Older tenants may still have the setting at the Client Connector Support page (global); newer tenants configure it per App Profile (== per Web Policy). When answering "why did this user get captive-portal-blocked and that user didn't on the same network", the answer is likely per-profile configuration, not the tenant-global FailOpenPolicy.
...
**Version-specific behavior** (from help captures — specific version floors where behavior changes):
...
- **Profile has no TRUSTED branch at all.** If `forwardingProfileActions` doesn't include an item with `networkType = TRUSTED`, ZCC treats all networks as untrusted regardless of criteria match. Valid configuration; the absence of a TRUSTED action block is functionally the same as `evaluate_trusted_network = false` (`forwarding_profile.go:40`).
...
- **QUIC / HTTP3 traffic bypasses ZIA proxy inspection.** ZCC intercepts traffic at the OS network layer. However, ZIA's forward proxy operates on TCP. QUIC runs over UDP 443 — if the forwarding profile allows UDP 443 to pass through (rather than dropping it), Safari and other QUIC-aggressive browsers route QUIC traffic to the internet without ZIA proxy inspection. Cloud App Control and URL Filtering rules are not evaluated for QUIC sessions because those rules require ZIA proxy interception. Result: no ZIA logs, original destination cert visible, policy rules don't fire. **Mitigation**: block UDP 443 at ZCC or at a network firewall upstream to force browsers to fall back to TCP. See [`../zia/cloud-app-control.md`](../zia/cloud-app-control.md) §QUIC. Compare forwarding profile config between tenants where QUIC is and isn't a problem — the diff is usually in how UDP 443 is handled.
