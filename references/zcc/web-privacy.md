...
- **Terraform support**: The vendored ZCC Terraform provider exposes `zcc_web_privacy`. Python SDK, Go SDK, and direct HTTP remain alternatives for automation, and direct API calls may still be needed for fields not modeled by a given SDK/provider version.
