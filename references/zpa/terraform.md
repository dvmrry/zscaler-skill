...
1. **`zia_cloud_domain` valid values** — The `zpa_zia_cloud_config.md` lists `zia_cloud_domain` as required and mentions "supported ZIA cloud names" but does not enumerate them. The allowed values are not documented in the provider source examined here.

2. **LSS `source_log_type` canonical list** — The eight alias docs cover most types but the mapping between internal API names and alias-doc filenames is inferred, not stated. Cross-reference against the `zpa_lss_config_log_type_formats` data source at runtime to get the current authoritative list.

3. **`app_connector_group` OAuth2 enrollment fields** — The `zpa_app_connector_group.md` references OAuth2 enrollment attributes (for ZIdentity-based connector enrollment) without fully documenting their schema. Field names and constraints should be confirmed from the provider Go source.

4. **Policy rule v1 deprecation timeline** — Both v1 (`zpa_policy_access_rule`) and v2 (`zpa_policy_access_rule_v2`) resources exist. The provider docs recommend v2 but do not specify a removal schedule. Verify with the provider changelog before migrating existing v1 rules.

5. **`zpa_private_cloud_group` vs `zpa_app_connector_group`** — The distinction between private cloud groups and standard app connector groups is not clearly documented. The `site_id` field on `zpa_private_cloud_group` suggests integration with a Site Controller, but the relationship is not detailed in the resource docs examined.

6. **Tag group membership in policy** — Tags and tag groups are listed as Early Access. Whether `zpa_tag_group` IDs can currently be referenced in policy rule conditions as an `object_type` is not stated in the resource docs.

7. **`zpa_policy_redirection_rule` CLIENT_TYPE values** — Only `CLIENT_TYPE` and `COUNTRY_CODE` are noted as valid condition types for redirection rules, but the allowable `CLIENT_TYPE` values (Zscaler Client Connector, ZPA App, etc.) are not enumerated in the doc. Use the `zpa_access_policy_client_types` data source to retrieve the current list.
