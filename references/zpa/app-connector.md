...
- The MCP create tool raises `ValueError("enrollment_cert_id is required for 'connector' key_type")` **only when `key_type == "connector"`**, leaving `service_edge` keys free to omit it (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zpa/provisioning_key.py:223-231`). The tool's own field annotation documents the parameter as "Enrollment certificate ID (required for 'connector')" (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zpa/provisioning_key.py:61-64`).
...
Deleting a Service Edge is destructive, and the edge must be re-provisioned to reconnect (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zpa/service_edges.py:181-201`).
