...
| `zpn_pbroker_comprehensive_stats` | Private Service Edge Metrics | No (format-only) | PSE telemetry. |
| `zpn_waf_http_exchanges_log` | AppProtection (Web Inspection) | Yes | App Protection policy activity. SDK key is `web_inspection`. |
| `zms_flow_log` | Microsegmentation | No (format-only) | Microsegmentation Flow activity. |
| `zpn_sitec_comprehensive_stats` | Private Cloud Controller Metrics | No (format-only) | Private Cloud Controller telemetry. |
| `zpn_sitec_auth_log` | Private Cloud Controller Status | No (format-only) | Private Cloud Controller availability. |
| `zpn_auth_log_1id` | User Status (Zidentity) *(inferred — see Open questions)* | No (format-only) | Zidentity-era variant of the User Status log. |
| `zpn_smb_inspection_log` | (SMB inspection) | No (format-only) | Inspection log; format lookup only. |
| `zpn_ldap_inspection_log` | (LDAP inspection) | No (format-only) | Inspection log; format lookup only. |
| `zpn_krb_inspection_log` | (Kerberos inspection) | No (format-only) | Inspection log; format lookup only. |
...
- **Format-only codes' receiver acceptance.** The 8 codes present in the TF format-lookup set but absent from the Python `source_log_map` (`zpn_pbroker_comprehensive_stats`, `zms_flow_log`, `zpn_sitec_comprehensive_stats`, `zpn_sitec_auth_log`, `zpn_auth_log_1id`, `zpn_smb_inspection_log`, `zpn_ldap_inspection_log`, `zpn_krb_inspection_log`) are confirmed valid for `GET /lssConfig/logType/formats`, but whether the ZPA API rejects them on a receiver's `sourceLogType` (vs. the Python SDK simply not exposing a key) is not established in available sources — the SDK validates against its own map, not a server response. (Tracked as [`zpa-41`](../_meta/clarifications.md#zpa-41-format-only-log-type-codes-acceptance-on-a-receivers-sourcelogtype).)
