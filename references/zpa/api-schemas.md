...
| Description | description | string | ✓ |  |
| Enabled | enabled | bool | ✓ |  |
| EnrollmentCertID | enrollmentCertId | string | ✓ | `omitempty` (`vendor/zscaler-sdk-go/zscaler/zpa/services/private_cloud_group/private_cloud_group.go:46`) |
| GeoLocationID | geoLocationId | string | ✓ |  |
...
`POST /zpa/mgmtconfig/v2/admin/customers/{customerId}/policySet/{policySetId}/rule` (create). `GET /zpa/mgmtconfig/v1/…/rule/{ruleId}` (single rule). `DELETE /zpa/mgmtconfig/v1/…/rule/{ruleId}`. All three sources agree: only POST (and PUT) differ by API version. (`vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontrollerv2/policysetcontrollerv2.go:227-233,240-247,254-279,286-292`, `vendor/zscaler-sdk-python/zscaler/zpa/policies.py:1617-1619,3966-3969`, `vendor/zscaler-api-specs/oneapi-postman-collection.json:73638,80514`)

**PolicyRuleResource (v1 response struct used by the v2 package) — field serialization notes.**
`microtenantId` has no omitempty on the response struct (always serialized). `conditions`, `appServerGroups`, `appConnectorGroups`, `serviceEdgeGroups` all lack omitempty on the response struct (always serialized). `credentialPool` has no omitempty on the response struct (a nil pointer serializes as null); on the request struct (`PolicyRule`) `credentialPool` does have omitempty. `policySetId` has `omitempty` on both `PolicyRule` (request) and `PolicyRuleResource` (response). (`vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontrollerv2/policysetcontrollerv2.go:47-95,124-166`)
...
**Process-level mutex on policy writes (v1/v2 coverage differs).**
Go has a package-level `sync.Mutex` (`ruleMutex`) in each package. v1 acquires it before `CreateRule`, `UpdateRule`, `Delete`, and `Reorder`, but not `BulkReorder` (`vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontroller/policysetcontroller.go:25,175-176,189-190,217-218,255-256,266-269`). v2 acquires it before all five write operations (`vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontrollerv2/policysetcontrollerv2.go:26,242-243,256-257,288-289,326-327,339-341`). GET operations skip the lock. Python SDK: `@synchronized(global_rule_lock)` module-level decorator on all add/update/delete/reorder methods (`vendor/zscaler-sdk-python/zscaler/zpa/policies.py:29-43,527`). The wrappers serialize the covered writes within their respective process/package; v1 `BulkReorder` is an exception.

**BulkReorder — Go SDK pins Default_Rule last; Python SDK does not.**
Go SDK `BulkReorder` (both v1 and v2) scans all rules, skips any rule named exactly `Default_Rule` (case-sensitive), sorts the rest by caller-supplied order map, then appends the `Default_Rule` ID at the end. Python `bulk_reorder_rules` sends the caller's `rules_orders` list unmodified. (`vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontroller/policysetcontroller.go:300-317`, `vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontrollerv2/policysetcontrollerv2.go:374-391`, `vendor/zscaler-sdk-python/zscaler/zpa/policies.py:4132-4147`)

**Bulk reorder — execute-once warning.**
Postman endpoint name states "Execute this API only once to reorder the rules". The endpoint replaces the entire rule order atomically from the submitted array. Sending it twice in quick succession risks a race. Go v2 `BulkReorder` holds `ruleMutex` during the call (`vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontrollerv2/policysetcontrollerv2.go:339-345`); the v1 implementation does not acquire the mutex (`vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontroller/policysetcontroller.go:266-269`). (`vendor/zscaler-api-specs/oneapi-postman-collection.json:75114`)

**v2 condition operand wire format.**
Operand: `{id, creationTime, modifiedBy, modifiedTime, objectType, values ([]string, omitempty), idpId, lhs, rhs, entryValues ([]OperandsResourceLHSRHSValue, omitempty)}`. Use `entryValues` for `POSTURE`, `TRUSTED_NETWORK`, `COUNTRY_CODE`, `PLATFORM`, `RISK_FACTOR_TYPE`, `SAML`, `SCIM`, `SCIM_GROUP`. Use `values[]` for `CLIENT_TYPE`, `APP_GROUP`, and similar. (`vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontrollerv2/policysetcontrollerv2.go:178-189`, `vendor/zscaler-sdk-python/zscaler/zpa/policies.py:171-187,300-316`)
...
**version field — declared by Postman, absent from the current Go v2 request.**
Postman declares a `version` field in its v2 policy-rule material, but the current Go v2 request struct `PolicyRule` has no `Version` field (`vendor/zscaler-api-specs/oneapi-postman-collection.json:73629`; `vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontrollerv2/policysetcontrollerv2.go:124-166`). The Go `Version` field at `policysetcontrollerv2.go:67` belongs to the v1 response struct `PolicyRuleResource`, not the v2 request. Do not add `version` to a Go v2 request based on the Postman declaration; whether the backend requires a version for a particular update flow remains unresolved from these sources.

**disabled field — string in Go SDK; integer in Postman.**
Both Go v1 and v2 declare `disabled` as string with omitempty. Postman v2 POST body types it as integer. Using boolean `true`/`false` will likely be rejected or misinterpreted. (`vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontroller/policysetcontroller.go:46`; `vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontrollerv2/policysetcontrollerv2.go:51,132`; `vendor/zscaler-api-specs/oneapi-postman-collection.json:73629`)

**reauthTimeout and reauthIdleTimeout — string in Go SDK; integer in Postman.**
Both fields are typed as string in both Go SDK packages (v1 and v2). Sample values: `'600'` (idle) and `'172800'` (48h total). Postman declares them as integer. Python SDK defaults: `reauth_timeout=172800`, `reauth_idle_timeout=600`; docstrings say str but defaults are int; the API appears to accept both. (`vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontroller/policysetcontroller.go:66-67`, `vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontrollerv2/policysetcontrollerv2.go:71-72,142-143`, `vendor/zscaler-sdk-python/zscaler/zpa/policies.py:1930-1931`, `vendor/zscaler-api-specs/oneapi-postman-collection.json:74375`)

**credential_id and credential_pool_id are mutually exclusive (Python SDK enforced).**
Exactly one must be supplied; both present or both absent returns an error tuple without calling the API. Wire format: `credential_id → {credential: {id: ...}}`, `credential_pool_id → {credentialPool: {id: ...}}`. Go SDK v2 `PolicyRule` has both `Credential *Credential` and `CredentialPool *Credential` (both omitempty on request struct; `CredentialPool` has no omitempty on response struct). (`vendor/zscaler-sdk-python/zscaler/zpa/policies.py:2920-2923`, `vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontrollerv2/policysetcontrollerv2.go:90-91,160-161`)
...
Python SDK calls `transform_common_id_fields` with `coerce_ids=False` in all ZPA policy methods. ZPA IDs are 19-digit strings that exceed JS Number precision (2^53). Converting to int is unsafe. Go SDK uses string type for the policy-rule ID in the v1 request and the v2 response/request (`vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontroller/policysetcontroller.go:43`; `vendor/zscaler-sdk-go/zscaler/zpa/services/policysetcontrollerv2/policysetcontrollerv2.go:48,125`).
...
**ApplicationServer.CreationTime — current JSON tag is corrected.**
At the refreshed Go pin the field is tagged `json:"creationTime"` with no
trailing comma, so the earlier claim that this struct currently emits a
`creationTime,` wire key is stale and must not be used for current payloads
(`vendor/zscaler-sdk-go/zscaler/zpa/services/appservercontroller/zpa_app_server_controller.go:18-31`).
