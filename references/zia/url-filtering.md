...
>
> **To pick the branch without guessing:** inspect the "No. of URLs Retaining Parent Category" / pattern / keyword counters on the Corp Exceptions category (admin console or `GET /api/v1/urlCategories/{id}` — fields `urlsRetainingParentCategoryCount`, `keywordsRetainingParentCategoryCount`, `patternsRetainingParentCategoryCount`, `ipRangesRetainingParentCategoryCount`). If logs exist, the `rule-hit-history` SPL pattern (see [`../shared/splunk-queries.md`](../shared/splunk-queries.md)) reveals which rule historically fired for the URL — fastest path to an empirical answer.
