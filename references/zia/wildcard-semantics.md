...
5. **Pure-TLD entries are rejected.** You cannot add a URL that's only a TLD — `.gov`, `.io`, `.com` etc. all fail validation. Operators trying to use TLD-blocking via custom categories must add multiple specific entries, or use Country / Category-based controls instead.
