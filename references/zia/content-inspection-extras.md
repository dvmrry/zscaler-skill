...
The Automate contract now provides a static captured file-type vocabulary for the rule body: `fileTypes` is a contract-only enum for `POST /zia/api/v1/fileTypeRules`, beginning with `ANY`, `NONE`, and `FTCATEGORY_*` values and ending at `FTCATEGORY_TS` in the generated reconciliation (`vendor/zscaler-api-specs/automate-zscaler/zia-divergences.json:5033-5036`, `:5093-5384`). Terraform does not carry a corresponding `fileTypes` choice list in the reconciled resource (`vendor/zscaler-api-specs/automate-zscaler/zia-divergences.json:5382`).
