...
| **Location Management `tz` field — read returns unprefixed; write requires `THE_` prefix for select countries.** All within the ZIA API. Verified via chain-of-evidence: write enum at `terraform-provider-zia/zia/validator.go:1402` (`THE_NETHERLANDS_EUROPE_AMSTERDAM`); ZIA TF resource validator commented out at schema level (`resource_zia_location_management.go:89`); Python SDK pass-through at `zscaler-sdk-python/.../models/location_management.py:43`; reproducible via Python read → YAML → ZIA Terraform apply workflow. See [`../_verification-protocol.md § Worked example`](../_verification-protocol.md). | [`./locations.md § Edge cases (tz prefix)`](./locations.md) | High — round-trip silently fails apply; affects only "the"-article country names |
