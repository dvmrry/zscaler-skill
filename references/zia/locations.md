...
7. **`tz` field — read returns unprefixed; write requires `THE_` prefix for select countries.** Same key, different value across read and write, all within the **ZIA API** (Python SDK reads ZIA, ZIA TF resource writes back to ZIA). Verified through a chain of vendored sources plus a reproduced workflow — see [`../_verification-protocol.md § Worked example`](../_verification-protocol.md) for the canonical demonstration of how this finding moved from tier-C operator report to tier-B verified.
   ...
   **Read-side stripping (tier B, reproducible chain — all ZIA):** the ZIA API's `GET /locations` response for a Netherlands-located tenant returns `tz` as `NETHERLANDS_EUROPE_AMSTERDAM` (no `THE_` prefix). Evidence chain:
   ...
   2. ZIA TF resource validator is commented out (above) — TF doesn't reject at plan-time; the value passes through to the ZIA API.
   3. Operator workflow: Python read → YAML emit → ZIA Terraform apply fails with API rejection of the unprefixed value (canonical write form per the ZIA validator file).
   4. Conclusion: since the SDK passes through verbatim and TF doesn't filter, the value rejected by the ZIA API on write is exactly what the ZIA API returned on read — confirming the asymmetry is in the ZIA API itself, not in any intermediate component.

   **Operational pattern:** when round-tripping a ZIA location through a Python-emitted YAML → ZIA Terraform pipeline (or any read-mutate-write flow against ZIA), normalize the `tz` field by re-prefixing `THE_` if the read value matches a country in the documented prefixed-form list at `validator.go:1402–1465`. Better: maintain a small mapping function in your tooling that translates read-side values to their canonical write-side enum.
   ...
   **What's also NOT verified** (still tier-C, tracked as a candidate): whether other "the"-article countries (Bahamas, Gambia, Philippines, etc.) exhibit the same read-vs-write asymmetry on `tz` against ZIA. The verified data point is Netherlands specifically; extrapolation to other countries is reasonable inference but not source-confirmed. Treat extension to other countries as tier-D inference until a second country is reproducibly observed.
