...
sources:
  ...
  - vendor/zscaler-sdk-go/zscaler/zia/services/firewallpolicies/dns_application_groups/dns_application_groups.go
...
**Bounded reverification (2026-09-23):** Python package-version references and request-executor/URL-category anchors affected by the Python drift, plus the Go Cloud App Control model addition, were checked at the current pins (`vendor/zscaler-sdk-python/pyproject.toml:3`; `vendor/zscaler-sdk-python/zscaler/request_executor.py:47-48`; `vendor/zscaler-sdk-python/zscaler/zia/url_categories.py:107-162`; `vendor/zscaler-sdk-go/zscaler/zia/services/cloudappcontrol/cloudappcontrol.go:43-45`). Other SDK catalog source families were not reverified, so `last-verified` remains 2026-08-12.
...
Government-cloud OneAPI support is client/version-specific. Current Go and Python SDK releases model FedRAMP OneAPI routing for `cloud=gov` / `cloud=govus`, using dedicated Zidentity auth domains and API gateways (`vendor/zscaler-sdk-go/zscaler/oneapiclient.go:412-446`; `vendor/zscaler-sdk-python/CHANGELOG.md:383-391`; `vendor/zscaler-sdk-python/zscaler/constants.py:21-28`). Older SDKs and non-ZIA provider paths may still require product-specific legacy auth.
...
**Go coverage:** CRUD is broadly parallel, but action discovery is asymmetric.

Go v3.8.46 has two action-discovery methods with the same request and flat
`[]string` response shape:

- `AvailableActions` posts to `/{rule_type}/availableActions`
  (`vendor/zscaler-sdk-go/zscaler/zia/services/cloudappcontrol/cloudappcontrol.go:223-251`).
- `AllAvailableActions` posts to the new
  `/{rule_type}/allAvailableActions` path (`cloudappcontrol.go:253-281`).

Python has only `list_available_actions` on the first path. The Go release
preserved the old exported `AllAvailableActions` name but changed which path it
selects, so an existing Go call can compile while changing endpoint behavior
after the v3.8.46 upgrade.

**Rule model field:** The current Go `WebApplicationRules` model includes `PromptCaptureEnabled` with JSON key `promptCaptureEnabled`; its source comment says the field applies only when Gen AI Applications Access is set to Allow (`vendor/zscaler-sdk-go/zscaler/zia/services/cloudappcontrol/cloudappcontrol.go:43-45`). The Python `CloudApplicationControl` parser and request formatter do not declare that field (`vendor/zscaler-sdk-python/zscaler/zia/models/cloudappcontrol.py:40-130,175-224`), so this typed Python model does not expose it.
...
**Wire-key divergence:** Go v3.8.44 serializes the Web-EUN flag as `isWebEUNEnabled`, matching recorded Python integration responses, but retains `omitempty` and therefore drops explicit `false` (`vendor/zscaler-sdk-go/CHANGELOG.md:122-129`; `vendor/zscaler-sdk-go/zscaler/zia/services/firewalldnscontrolpolicies/firewalldnscontrolpolicies.go:151-160`; `vendor/zscaler-sdk-python/tests/integration/zia/cassettes/TestCloudFirewallDNSRules.yaml:26,104,186`). Current Python v1.9.44 (`vendor/zscaler-sdk-python/pyproject.toml:3`) still parses and emits `isWebEunEnabled`, including that spelling in its request dictionary (`vendor/zscaler-sdk-python/zscaler/zia/models/cloud_firewall_dns_rules.py:57-58,250-264`). The server-side effect of omitted `false` versus an explicitly supplied false value is unverified; do not assume the two clients have equivalent update semantics.
...
| `get_pac_file` | `(pac_id: int, query_params=None)` | GET `/pacFiles/{id}/version`; despite the singular name, returns all versions under that PAC-file parent. |
| `get_pac_file_version` | `(pac_id: int, pac_version: int, query_params=None)` | GET `/pacFiles/{id}/version/{version}`. |
| `add_pac_file` | `(**kwargs)` | POST `/pacFiles`. |
| `update_pac_file` | `(pac_id: int, pac_version: int, pac_version_action: str, new_lkg_ver=None, **kwargs)` | PUT `/pacFiles/{id}/version/{version}/action/{action}`. Python serializes the kwargs, including `pac_commit_message`, as an object body. |
| `delete_pac_file` | `(pac_id: int)` | DELETE `/pacFiles/{id}`. |
| `validate_pac_file` | `(pac_file_content: str) -> APIResult[PacFileValidationResponse]` | POST `/pacFiles/validate`; sends PAC source as the raw body. |
...
**Go parity:** No for the version-action request body.

Go v3.8.46 deliberately diverges on the version-action body:
`UpdatePacFile(..., commitMessage string, newLKGVer *int)` sends the commit
message as the raw request body because the SDK source says an object corrupts
the stored message
(`vendor/zscaler-sdk-go/zscaler/zia/services/pacfiles/pacfiles.go:194-217`).
Python v1.9.44 (`vendor/zscaler-sdk-python/pyproject.toml:3`) still builds an object from kwargs
(`vendor/zscaler-sdk-python/zscaler/zia/pac_files.py:509-574`) and its request
executor assigns that body to the JSON payload slot
(`vendor/zscaler-sdk-python/zscaler/request_executor.py:347-356`). The captured
Automate operation declares the same PUT path but no request body
(`vendor/zscaler-api-specs/automate-zscaler/zia-api-reference.json:368985-369041`).
This is a three-way contract divergence; do not treat the signatures as
payload-compatible or infer Python failure without a live request trace.
...
| `list_categories_lite` | `(query_params=None)` | GET `/urlCategories/lite`; added in Python v1.9.40 and returns the lightweight category collection (`vendor/zscaler-sdk-python/CHANGELOG.md:51-60`; `vendor/zscaler-sdk-python/zscaler/zia/url_categories.py:107-162`). |
...
| `get_url_and_app_settings` | `()` | GET `/advancedUrlFilterAndCloudAppSettings`. Returns `AdvancedUrlFilterAndCloudAppSettings` (`vendor/zscaler-sdk-python/zscaler/zia/url_filtering.py:430-469`). |
| `update_url_and_app_settings` | `(**kwargs)` | PUT `/advancedUrlFilterAndCloudAppSettings`; kwargs are sent as a sparse body (`vendor/zscaler-sdk-python/zscaler/zia/url_filtering.py:471-558`). |

**Advanced-settings caveat:** Go v3.8.45 models all 14 GenAI prompt flags and
serializes nearly every boolean even when false, so its PUT should be treated as
a GET-modify-PUT full-state operation. Python's typed response model covers only
eight prompt flags, although its kwargs request path can pass additional fields
through after camel-case conversion. The omitted Mistral flag is a special
case: use exact `enableMistralAIPrompt`, because inferred
`enable_mistral_ai_prompt` becomes the wrong `enableMistralAiPrompt`. See
[`api-divergences.md`](./api-divergences.md#advanced-urlcloud-app-settings-have-asymmetric-sdk-models-and-update-semantics).
...
### Service surfaces added before Python v1.9.39
...
### Python v1.9.39 service expansion

Python v1.9.39 introduced thirteen additional services that remain on the
current unified `ZIAService` surface (`vendor/zscaler-sdk-python/CHANGELOG.md:62-143`;
`vendor/zscaler-sdk-python/zscaler/zia/zia_service.py:841-943`):

| Accessor | Methods and endpoint scope |
|---|---|
| `dns_application_groups` | `list_groups`, `get_group`, `add_group`, `update_group`, `delete_group` over `/zia/api/v1/dnsApplicationGroups[/{id}]` (`vendor/zscaler-sdk-python/zscaler/zia/dns_application_groups.py:37-263`). |
| `endpoint_applications` | Six reads for `/endPointApplications`, `/lite`, `/count`, `/cloudApps/count`, `/policies`, and `/getCategoriesWithNonEmptyApps` (`vendor/zscaler-sdk-python/zscaler/zia/endpoint_applications.py:39-381`). |
| `endpoint_custom_apps` | List/get/create/update/delete for `/endPointApplications/customApps` and `/customApp[/{id}]` (`vendor/zscaler-sdk-python/zscaler/zia/endpoint_custom_apps.py:38-299`). |
| `endpoint_application_groups` | Group list/CRUD plus resource-association update and policy-association read under `/endPointApplicationGroups` (`vendor/zscaler-sdk-python/zscaler/zia/endpoint_application_groups.py:38-320`). |
| `dlp_endpoint_resource` | Create/update/delete and channel-scoped list/get operations under `/dlpEndpointResource` (`vendor/zscaler-sdk-python/zscaler/zia/dlp_endpoint_resource.py:37-190,192-261,263-287`). |
| `endpoint_dlp_resource_groups` | Resource-group CRUD, resource-to-group reads, channel-scoped group reads, and group-resource read/update operations (`vendor/zscaler-sdk-python/zscaler/zia/endpoint_dlp_resource_groups.py:38-179,181-235,237-307,309-367,369-392`). |
| `endpoint_dlp_rules` | Rule list/get/CRUD plus the `/fileTypeCategories` read (`vendor/zscaler-sdk-python/zscaler/zia/endpoint_dlp_rules.py:37-345,347-389`). |
| `endpoint_dlp_sub_rules` | Exception/sub-rule create, update, and delete under `/endPointDlpRules/{id}/subRule` (`vendor/zscaler-sdk-python/zscaler/zia/endpoint_dlp_sub_rules.py:35-125,127-209,211-235`). |
| `outbound_email_dlp_rules` | List/get/lite-list/CRUD for `/emailDlpRules` plus the raw actions download at `/emailDlpRules/actions` (`vendor/zscaler-sdk-python/zscaler/zia/outbound_email_dlp_rules.py:37-202,204-402,404-456`). |
| `end_user_notification_templates` | Five reads covering EUN templates, EUN feature status, user-confirmation templates by product, global defaults, and confirmation feature status (`vendor/zscaler-sdk-python/zscaler/zia/end_user_notification_templates.py:39-291`). |
| `web_dlp_global_options` | GET and PUT `/webDlpGlobalOptions` (`vendor/zscaler-sdk-python/zscaler/zia/web_dlp_global_options.py:37-112`). |
| `ips_categories` | `list_categories` reads `/ipsCategories` (`vendor/zscaler-sdk-python/zscaler/zia/ips_categories.py:37-103`). |
| `nss_collectors` | `list_collectors` reads `/nssCollectors` (`vendor/zscaler-sdk-python/zscaler/zia/nss_collectors.py:37-92`). |

These services are unified-client surfaces. Of this v1.9.39 batch, the legacy
client registers only `web_dlp_global_options`
(`vendor/zscaler-sdk-python/zscaler/zia/legacy.py:786-792`); the complete batch
is registered on `ZIAService`
(`vendor/zscaler-sdk-python/zscaler/zia/zia_service.py:841-943`). The new Python
list methods return the page in `response.get_results()` after one request; they
do not aggregate subsequent pages automatically, as shown by DNS application
groups, endpoint applications, and Outbound Email DLP
(`vendor/zscaler-sdk-python/zscaler/zia/dns_application_groups.py:65-92`;
`vendor/zscaler-sdk-python/zscaler/zia/endpoint_applications.py:79-106`;
`vendor/zscaler-sdk-python/zscaler/zia/outbound_email_dlp_rules.py:75-102`).

### Cross-SDK July 2026 ZIA expansion

Go v3.8.41 and Python v1.9.39 now both expose Endpoint DLP application catalog/count/category/policy reads, custom-application CRUD, application-group CRUD and associations, channel-scoped Endpoint DLP resource management, resource-group CRUD/associations, Endpoint DLP rule and exception/sub-rule management, Outbound Email DLP list/lite/get/CRUD plus actions CSV, DNS application-group CRUD, and five EUN/user-confirmation template or feature-status reads (`vendor/zscaler-sdk-go/CHANGELOG.md:179-258`; `vendor/zscaler-sdk-python/CHANGELOG.md:62-143`; `vendor/zscaler-sdk-python/zscaler/zia/outbound_email_dlp_rules.py:37-456`). Both also expose `GET /zia/api/v1/ipsCategories` (`vendor/zscaler-sdk-go/zscaler/zia/services/ips_control_policies/ips_signature_rules/ips_signature_rules.go:14-19,186-194,307-313`; `vendor/zscaler-sdk-python/zscaler/zia/ips_categories.py:37-103`).

Neither changelog is a complete endpoint inventory. The Go release notes mention only `/emailDlpRules/actions` while code includes full Outbound Email DLP CRUD/list/lite/get, and they omit `/ipsCategories` (`vendor/zscaler-sdk-go/CHANGELOG.md:242-257`; `vendor/zscaler-sdk-go/zscaler/zia/services/endpoint_dlp/outbound_email_dlp/outbound_email_dlp.go:57-160`; `vendor/zscaler-sdk-go/zscaler/zia/services/ips_control_policies/ips_signature_rules/ips_signature_rules.go:307-313`). Python 1.9.39 likewise lists only the actions download for Outbound Email DLP and omits both the IPS-category and NSS-collector reads, although all three surfaces are present in code (`vendor/zscaler-sdk-python/CHANGELOG.md:62-114`; `vendor/zscaler-sdk-python/zscaler/zia/outbound_email_dlp_rules.py:37-456`; `vendor/zscaler-sdk-python/zscaler/zia/ips_categories.py:37-103`; `vendor/zscaler-sdk-python/zscaler/zia/nss_collectors.py:37-92`).
...
Source: `vendor/zscaler-sdk-python/zscaler/zia/audit_logs.py`.

**zia-sdk-01** — Resolved 2026-04-26. `AuditLogsAPI` return convention is intentionally non-standard. Each method returns a raw value rather than a `(result, response, error)` tuple: `get_status()` returns `response.get_body()` (dict); `create()` returns `response.get_status()` (int HTTP status code); `cancel()` returns `response.status_code` (int); `get_report()` returns `response.get_body()` (str CSV). All four return `None` on either request creation error or execution error — errors are indistinguishable from a successful empty response. This is a known limitation of the legacy-style `AuditLogsAPI` implementation; the async report pattern (POST → poll → download) doesn't map cleanly to the standard tuple.

Source: `vendor/zscaler-sdk-python/zscaler/zia/browser_control_settings.py` line 25.

**zia-sdk-02** — Resolved 2026-04-26. `BrowserControlSettingsPI` class name typo confirmed. Class is named `BrowserControlSettingsPI` (missing the `A`). All imports in `vendor/zscaler-sdk-python/zscaler/zia/zia_service.py` and `vendor/zscaler-sdk-python/zscaler/zia/legacy.py` use this same incorrect name. Callers using the `client.zia.browser_control_settings` accessor are unaffected; the typo is only visible when importing the class directly.

Source: `vendor/zscaler-sdk-python/zscaler/zia/dns_gatways.py`.

**zia-sdk-03** — Resolved 2026-04-26. `dns_gatways.py` filename typo confirmed. The file exists with the misspelled name. The `ZIAService` accessor imports from this file and exposes it as `dns_gateways` (correctly spelled), so all callers via `client.zia.dns_gateways` are unaffected. Direct file imports would require the misspelled path.
