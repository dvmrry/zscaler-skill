...
**zia-sdk-01** — `AuditLogsAPI` return convention  
`AuditLogsAPI.get_status()`, `create()`, `cancel()`, and `get_report()` do not return the standard `(result, response, error)` tuple. They return raw values (`dict`, `int`, `str`) or `None` on error. This breaks the SDK convention and makes error handling inconsistent. It is unclear whether this is intentional or a legacy artifact. Callers cannot distinguish API errors from missing data.

**zia-sdk-02** — `BrowserControlSettingsPI` class name typo  
The class in `browser_control_settings.py` is named `BrowserControlSettingsPI` (missing the `A`). This appears to be a typo. The accessor in `ZIAService` is wired correctly but the class name is inconsistent with all other ZIA API classes.

**zia-sdk-03** — `dns_gatways.py` filename typo  
The file is named `dns_gatways.py` (missing the `e` in `gateways`). The accessor in `ZIAService` resolves this transparently. Callers using the service accessor are unaffected.

**zia-sdk-04** — `AdminRolesAPI.update_password_expiry_settings` endpoint mismatch  
The `update_password_expiry_settings` method in `admin_roles.py` sends a PUT to `/cyberThreatProtection/advancedThreatSettings` rather than `/passwordExpiry/settings`. This looks like a copy-paste error from `ATPPolicyAPI.update_atp_settings`. The correct endpoint for password expiry settings is unverified from source code alone.

**zia-sdk-05** — `SecurityPolicyAPI` whitelist/blacklist URL mutation endpoints  
The `add_urls_to_whitelist` and `delete_urls_from_whitelist` methods reference `/security/advanced/blacklistUrls` in source code comments. The exact HTTP method and action query parameter for each operation is not visible in the 50-line read window and warrants verification against the live API.

**zia-sdk-06** — `TrafficDatacentersAPI` vs `TrafficForwardingGRETunnelAPI` VIP overlap  
Both `traffic_datacenters.py` and `gre_tunnel.py` expose `list_vips` and `list_vips_by_datacenter`. The two implementations may differ in query parameters or response models. The canonical location for VIP listing is unclear.

**zia-sdk-07** — Go SDK parity for newer services  
Services added after the initial SDK release (CASB DLP/Malware rules, custom file types, dedicated IP gateways, DNS gateways, risk profiles, sandbox rules, sub-clouds, system audit, traffic extranet, ZPA gateway) do not appear in the Go SDK services list. It is possible the Go SDK has since been updated; the parity flags in this document reflect the state of `vendor/zscaler-sdk-go/zscaler/zia/services/` at the time of review.

**zia-sdk-08** — `DLPResourcesAPI` IDM profile endpoint  
The `list_idm_profiles` and `get_idm_profile` methods use `/idmprofile` as the base path. The IDM (Indexed Document Matching) profile API is not well documented in the SDK source and its model structure is not visible in the reviewed files.
