...
| App Profile (ZCC) | Device Profile, ZCC App Profile | — | Exposed as `client.zcc.application_profiles` (`/application-profiles`, list + get-by-id + PATCH) in both SDKs (Go: `application_profiles.go:296`; Python: `zcc_service.py:129-134`). Also surfaces as `WebPolicy` on the legacy `/web/policy` path. See `clarification zcc-07`. |
...
| Zero Trust Gateway (ZTG) | — | Admin console surfaces include `Zero Trust Gateway -- AWS` and `Zero Trust Gateway -- GCP` | Zscaler-managed, cloud-native gateway service distinct from the customer-deployed Cloud Connector VM. Current cloud-specific Help documents AWS and GCP as Limited Availability and Support-enabled; exact Azure deployment availability remains unresolved (`vendor/zscaler-help/cbc-about-amazon-web-services-zero-trust-gateways.md:8-21`; `vendor/zscaler-help/cbc-about-google-cloud-platform-zero-trust-gateways.md:8-17`; see `references/cloud-connector/overview.md`). |
...
| Bring Your Own IP (BYOIP) | Customer-owned Dedicated IP | ZIA Dedicated IP | Support-assisted use of a customer-owned, ROA-authorized prefix in Zscaler's Dedicated IP service. Requires at least a `/24` for each requested Zscaler data center. It is distinct from SIPA, which uses a ZPA App Connector as the observed source address (`vendor/zscaler-help/zia-dedicated-ip-using-customer-owned-ip-addresses.md:8-49`; `vendor/zscaler-help/understanding-source-ip-anchoring.md:18-28`). |
...
| AI Guard | ZGuard (`client.zguard` is a deprecated Python accessor alias) | Current Help root: `secure-ai-users/`; former root `ai-guard/` returned a maintenance status in the current capture (`vendor/zscaler-help/ai-guard-users-help-index.md:50-55`); Python package `zscaler/aiguard/`; canonical accessor `client.aiguard` | LLM runtime protection for prompt/response policy detection. Python SDK 1.9.39 exposes six OneAPI configuration resources under `client.aiguard`, while `LegacyAIGuardClient(...).aiguard.policy_detection` routes runtime detection; this client split is not a universal backend/auth constraint (`vendor/zscaler-sdk-python/zscaler/oneapi_client.py:343-385`, `:671-712`; `vendor/zscaler-sdk-python/zscaler/aiguard/aiguard_service.py:26-84`). Deployment modes remain Proxy, DaaS, and OnPrem. See `references/ai-security/ai-guard.md`. |
| AI Access Security | Secure Access to AI Apps | Product page: `products-and-solutions/ai-access-security` | Current positioning surface for controlling access and content across public GenAI, embedded SaaS AI, agents, and developer tools, including discovery, prompt/response insight, access controls, inline DLP, moderation, and AI-IDE controls. Publication does not prove tenant entitlement, rollout stage, API parity, or cloud availability (`vendor/zscaler-help/ai-access-security-marketing.md:8-31`). |
