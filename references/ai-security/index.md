...
The current public Automate operation pages now enumerate **114 `ai-security` operations**: **17 read-only AI Security inventory/findings operations** plus 97 AI Red Teaming operations. The 17 inventory operations cover agents, code repositories, data stores, guardrails, identities, issues, MCP servers/tools, and workloads; the operation map and direct citations are maintained in [`./asset-management-api.md`](./asset-management-api.md). The durable 2026-08-12 capture described below records the earlier 108-operation composition (11 inventory plus 97 Red Teaming) and should be read as a baseline. The current public operation pages also publish the 47-operation AI Guard admin set, while that durable capture recorded no `aiguard` routes. This publication change does not establish entitlement, runtime acceptance, or endpoint retirement; Python still exposes 39 callable AI Guard configuration methods. Representative current pages include [list agents](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/ai-security/aisecurity/v1-assets-agents/agents-list-agents), [list guardrails](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/ai-security/aisecurity/v1-resources-guardrails/guardrails-list-guardrails), and [disable detection policy](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/aiguard/detection-policies/detections-policy-resource-disable-detections-policy).
