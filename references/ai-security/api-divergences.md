...
The current public Automate operation pages publish the **47 AI Guard operations across 29 paths** represented by the current contract. The current comparison matches all 47 operations with zero schema-changed operations and records eight colon-action route corrections; the capture reports no retained publication absences (`vendor/zscaler-api-specs/automate-zscaler/openapi-validation-report.md:7-10`; `vendor/zscaler-api-specs/automate-zscaler/docusaurus-snapshot-compare-summary.md:9-21`, `:27-28`, `:47-48`, `:119-126`; current route examples at `vendor/zscaler-api-specs/automate-zscaler/aiguard-api-reference.json:477-489`, `:609-621`, `:4021-4033`). The earlier [2026-08-12](https://github.com/dvmrry/zscaler-skill/blob/3174c9ac8901b019bc96de5f60af77677f52841d/vendor/zscaler-api-specs/automate-zscaler/docusaurus-snapshot-compare-summary.md#L3-L64) no-AI-Guard observation remains dated historical context only; it is not a current publication absence, endpoint-retirement, backend-availability, or live-acceptance conclusion. Current citations include [disable detection policy](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/aiguard/detection-policies/detections-policy-resource-disable-detections-policy), [referential-check detection policy](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/aiguard/detection-policies/detections-policy-resource-referential-check-detections-policy), and [regenerate LLM application credential](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/aiguard/llm-application-credentials/llm-application-credentials-resource-regenerate-llm-application-credentials). Current publication still does not establish endpoint entitlement, live acceptance, or backend availability.
...
Two read-only provider-type discovery operations are present in the current contract:
...
That does **not** mean the client surfaces have full parity. Python wraps 39 of the 47 operations in the current public set; the same pass still found no matching Go SDK service, Terraform resource, MCP tool, Postman endpoint, or Automation Hub procedure in the inspected client/source classes. Treat the current operation pages and current raw contract as documented method/path/field evidence, and each client inventory as its own coverage boundary. Do not infer tenant entitlement, live endpoint acceptance, endpoint retirement, or backend unavailability from the wrapper inventory or from changes in route publication. See [clarification ai-security-04](../_meta/clarifications.md#ai-security-04-ai-guard-admin-plane-programmability) for the broader wrapper boundary and [clarification ai-security-08](../_meta/clarifications.md#ai-security-08-python-sdk-ai-guard-admin-operation-gap) for the Python gap.

The current contract supersedes the earlier adjacent-template slash spelling: enable/disable are `/v1/detections/policies/{id}:enable` and `/v1/detections/policies/{id}:disable`, referential checks use `/{id}:referential-check`, and credential rotation uses `/{id}:regenerate` (`vendor/zscaler-api-specs/automate-zscaler/aiguard-api-reference.json:477-489`, `:609-621`, `:1469-1481`, `:4021-4033`; eight route corrections at `vendor/zscaler-api-specs/automate-zscaler/docusaurus-snapshot-compare-summary.md:119-126`). The previous `ai-security-05` clarification remains closed for the current documented shape; current publication, live authorization, and tenant acceptance are separate concerns.

### Python SDK coverage gap

Eight Automate-documented operations are not callable in the Python inventory: policy enable, disable, referential check, and summaries, plus referential checks for providers, provider credentials, applications, and application credentials. The current comparison records eight colon-action route corrections; current raw operations and the Python inventory remain separate evidence layers (`vendor/zscaler-api-specs/automate-zscaler/docusaurus-snapshot-compare-summary.md:27-28`, `:47-48`, `:119-126`; `vendor/zscaler-api-specs/automate-zscaler/aiguard-api-reference.json:477-489`, `:609-621`, `:1469-1481`, `:1954-1966`, `:3956-3968`, `:4021-4033`, `:5229-5241`, `:6142-6154`, `:7166-7178`; Python inventories above). The application-credential regenerate action is callable in Python, but v1.9.44 constructs the slash path `/llm-application-credentials/{credential_id}/regenerate` (`vendor/zscaler-sdk-python/zscaler/aiguard/llm_application_credentials.py:290-333`, especially `:308-312`) while the current Automate contract publishes `/llm-application-credentials/{id}:regenerate` (`vendor/zscaler-api-specs/automate-zscaler/aiguard-api-reference.json:4021-4033`). Callable presence therefore is not wire-path parity; live acceptance of the remaining path divergence is open.
...
The current enable/disable response is a `PolicyControlUpdateResult`: `enabled`, affected `matchRules[]` (`id`, `name`, `enabled`), `policyId`, and `updatedCount` (`vendor/zscaler-api-specs/automate-zscaler/aiguard-api-reference.json:505-592`, `:637-724`). This is current documented shape, not proof of live tenant response behavior.

The current LLM Application create/update/read schemas do not expose `defaultPolicyId` or `applicationSettings.customerManagedKey.{kmsKeyId,kmsProviderType}` (`vendor/zscaler-api-specs/automate-zscaler/aiguard-api-reference.json:4464-4712`, `:4768-5073`, `:5294-5542`; the current comparison reports zero AI Guard schema changes at `vendor/zscaler-api-specs/automate-zscaler/docusaurus-snapshot-compare-summary.md:47-48`). Treat the omission as a current contract boundary only. It does not prove that the backend removed the fields, that existing stored values were deleted, or that tenant-level customer-managed-key configuration was removed.

## Help provider labels vs Automate provider types

The 2026 Help chronology names supported applications/providers that do not all appear in the current 23-value Automate `type` enum. Examples include GitHub Copilot, ElevenLabs, Windsurf, Mistral Vibe, Gamma, and Builder.io (`vendor/zscaler-help/ai-guard-release-upgrade-summary-2026.md:14-50`; current Automate enum at `vendor/zscaler-api-specs/automate-zscaler/aiguard-api-reference.json:7503-7533`). These may represent User-mode application labels, built-in providers that are not customer-created admin objects, or documentation/API timing differences. Static sources do not establish a one-to-one mapping; track the boundary under [clarification ai-security-07](../_meta/clarifications.md#ai-security-07-help-provider-labels-vs-automate-provider-types).
