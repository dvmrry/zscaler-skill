...
The current public Automate operation pages enumerate **17 `GET` operations** under `https://api.zsapi.net/aisecurity/aispm`: list/get agents, code repositories, data stores, guardrails, identities, issues, MCP servers, and workloads, plus list tools for a discovered MCP server. They are the inventory/findings subset of the **114-operation `ai-security` publication**, alongside 97 AI Red Teaming operations. The durable 2026-08-12 OpenAPI captured 11 inventory operations and 108 combined operations as the prior baseline; it does not supersede the current operation pages. This remains distinct from AI Guard's prompt/response runtime and admin contract. See [`./asset-management-api.md`](./asset-management-api.md) for the operation map, cursor semantics, current schema drift, and client-wrapper boundary. Current citations include [list agents](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/ai-security/aisecurity/v1-assets-agents/agents-list-agents), [list code repositories](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/ai-security/aisecurity/v1-resources-coderepositories/code-repositories-list-code-repositories), [list guardrails](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/ai-security/aisecurity/v1-resources-guardrails/guardrails-list-guardrails), and [list workloads](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/ai-security/aisecurity/v1-resources-workloads/workloads-list-workloads).
...
The configuration surface introduced in Python SDK 1.9.39 remains present in current v1.9.44, exposing six OneAPI resources with 39 callable methods, while the separate `LegacyAIGuardClient` retains runtime policy detection (`vendor/zscaler-sdk-python/CHANGELOG.md:141-206`; current version at `vendor/zscaler-sdk-python/pyproject.toml:3`; `vendor/zscaler-sdk-python/zscaler/oneapi_client.py:343-385`, `:671-712`; `vendor/zscaler-sdk-python/zscaler/aiguard/aiguard_service.py:26-84`; method inventories cited above). Against the current public 47-operation AI Guard Automate set, eight documented actions remain outside the callable Python inventory. The current comparison matches all 47 operations with zero schema-changed operations and records eight colon-action route corrections (`vendor/zscaler-api-specs/automate-zscaler/docusaurus-snapshot-compare-summary.md:27-28`, `:47-48`, `:119-126`; current route examples at `vendor/zscaler-api-specs/automate-zscaler/aiguard-api-reference.json:477-489`, `:609-621`, `:4021-4033`). The earlier [2026-08-12](https://github.com/dvmrry/zscaler-skill/blob/3174c9ac8901b019bc96de5f60af77677f52841d/vendor/zscaler-api-specs/automate-zscaler/docusaurus-snapshot-compare-summary.md#L3-L64) route table's no-AI-Guard observation is historical context only; the current capture reports no retained publication absences, and neither observation establishes tenant entitlement, endpoint retirement, backend availability, or live acceptance. Current operation-page examples include [disable detection policy](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/aiguard/detection-policies/detections-policy-resource-disable-detections-policy), [enable detection policy](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/aiguard/detection-policies/detections-policy-resource-enable-detections-policy), and [regenerate application credential](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/aiguard/llm-application-credentials/llm-application-credentials-resource-regenerate-llm-application-credentials).
...
The current public Automate route table exposes **97 AI Red Teaming operations** under the `ai-security/airedteaming` route family, in addition to the 17 current AI Security asset/findings operations. The current operation inventory spans AI apps and integration tests, business units, file uploads, model benchmarks, probes and probe runs, remediation and policy generation, reports, scheduled tests, test-case results, test runs, and trigger operations (`vendor/zscaler-api-specs/automate-zscaler/openapi/ai-security.openapi.json:18-34`; normalized operation inventory at `vendor/zscaler-api-specs/automate-zscaler/ai-security-api-reference.json:2-20832`). The combined OpenAPI lists the Red Teaming service URL `https://api.zsapi.net/aisecurity/airt` separately from the asset-management URL `https://api.zsapi.net/aisecurity/aispm` (`vendor/zscaler-api-specs/automate-zscaler/openapi/ai-security.openapi.json:49186-49202`). The durable [2026-08-12](https://github.com/dvmrry/zscaler-skill/blob/3174c9ac8901b019bc96de5f60af77677f52841d/vendor/zscaler-api-specs/automate-zscaler/docusaurus-snapshot-compare-summary.md#L3-L64) source records the earlier 11+97 composition as a dated baseline; current inventory operation citations are maintained in [`./asset-management-api.md`](./asset-management-api.md).
...
2. **AI Guard has a substantial but incomplete Python SDK surface.** The canonical `client.aiguard` accessor exposes six OneAPI configuration resources; `client.zguard` is only a deprecated alias, and runtime detection stays on `LegacyAIGuardClient(...).aiguard.policy_detection` (`vendor/zscaler-sdk-python/zscaler/oneapi_client.py:343-385`, `:671-712`; `vendor/zscaler-sdk-python/zscaler/aiguard/aiguard_service.py:26-84`). Python has 39 callable configuration methods versus 47 operations in the current public Automate operation set. The current comparison matches all 47 operations with zero schema-changed operations and records eight colon-action route corrections; the eight-operation wrapper gap remains open (`vendor/zscaler-api-specs/automate-zscaler/docusaurus-snapshot-compare-summary.md:27-28`, `:47-48`, `:119-126`; current route examples at `vendor/zscaler-api-specs/automate-zscaler/aiguard-api-reference.json:477-489`, `:609-621`, `:4021-4033`). The earlier [2026-08-12](https://github.com/dvmrry/zscaler-skill/blob/3174c9ac8901b019bc96de5f60af77677f52841d/vendor/zscaler-api-specs/automate-zscaler/docusaurus-snapshot-compare-summary.md#L3-L64) route table's no-AI-Guard observation is historical context only; the current capture reports no retained publication absences. Do not present either the wrapper gap or a publication change as proof of tenant entitlement, endpoint retirement, backend unavailability, or Terraform coverage. Representative current pages include [disable detection policy](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/aiguard/detection-policies/detections-policy-resource-disable-detections-policy), [enable detection policy](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/aiguard/detection-policies/detections-policy-resource-enable-detections-policy), and [regenerate application credential](https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/aiguard/llm-application-credentials/llm-application-credentials-resource-regenerate-llm-application-credentials).
