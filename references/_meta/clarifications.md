...
**2026-06-18 narrowing**: the Automate contract now gives a static category-level `actions` vocabulary for `POST /zia/api/v1/webApplicationRules/:rule_type`, including the generated contract-only action list in the ZIA divergence report (`vendor/zscaler-api-specs/automate-zscaler/zia-divergences.json:2287-2290`, `:2332-2527`). That closes the "what is the captured documented action vocabulary?" part. It does **not** close per-app validity: the contract still describes a rule body, not a read path that maps each app to its individually accepted actions.
...
**2026-06-18 narrowing**: the Automate contract now corroborates a broad category-level Cloud App Control action vocabulary on `webApplicationRules/:rule_type`, with `actions` represented as a contract-only enum in the generated ZIA divergence report (`vendor/zscaler-api-specs/automate-zscaler/zia-divergences.json:2287-2290`, `:2332-2527`). That improves the static action-vocabulary footing, but the atomic multi-app rejection and representative-app behaviors remain workflow-skill observations until tested against a tenant.
...
The reconstructed OpenAPI snapshot carries 165 ZCloudConnector operations, but the structural validation report flags 124 of them as `default_response_as_success`: the exporter used a schema-bearing `default` response as the success schema because no explicit `2xx` schema was present (`vendor/zscaler-api-specs/automate-zscaler/openapi-validation-report.md:16`, `vendor/zscaler-api-specs/automate-zscaler/openapi-validation-report.md:24`, `vendor/zscaler-api-specs/automate-zscaler/openapi-validation-report.md:29-152`). This preserves useful method/path/field metadata for reconciliation, but it does not prove the intended success status code for those operations or whether the embedded `default` response is a success envelope rather than a catch-all envelope.
...
The reconstructed Automate contract publishes alert and user snapshot creation as `POST /snapshot/alert` and `POST /snapshot/user`, and the snapshot validation flags both as path-prefix anomalies relative to the usual ZDX `/v1` path prefix (`vendor/zscaler-api-specs/automate-zscaler/zdx-api-reference.json:91526-91538`, `vendor/zscaler-api-specs/automate-zscaler/zdx-api-reference.json:91675-91687`, `vendor/zscaler-api-specs/automate-zscaler/openapi-validation-report.md:153-154`). The Python SDK composes the alert snapshot method with `_zdx_base_endpoint = "/zdx/v1"` plus `/snapshot/alert` (`vendor/zscaler-sdk-python/zscaler/zdx/snapshot.py:27`, `vendor/zscaler-sdk-python/zscaler/zdx/snapshot.py:83-87`). Which base path a live OneAPI tenant accepts for the snapshot routes is not confirmed by static sources.
...
The 2026-06-16 refresh found Python SDK runtime policy detection and public DaaS integration examples, but no broad AI Guard admin-plane client wrapper, Go SDK service, Terraform resource, MCP tool, Postman endpoint, or Automation Hub procedure in the inspected source classes. The 2026-07-20 Docusaurus snapshot narrowed that by exposing 47 Automate operations across detection policies, match rules, applications, providers, provider types, and credentials (retained provider-type discovery at `vendor/zscaler-api-specs/automate-zscaler/aiguard-api-reference.json:7486-7903`). The 2026-08-12 public route table no longer publishes AI Guard, so the 47-operation snapshot is retained as last-known evidence; publication absence does not establish endpoint retirement or backend unavailability (`vendor/zscaler-api-specs/automate-zscaler/docusaurus-snapshot-compare-summary.md:19-23`; `vendor/zscaler-api-specs/automate-zscaler/openapi-validation-report.md:10`, `:156-160`). Python SDK v1.9.39 added six OneAPI configuration resources with 39 callable methods (`vendor/zscaler-sdk-python/zscaler/aiguard/aiguard_service.py:26-84`; method inventories in `vendor/zscaler-sdk-python/zscaler/aiguard/policies.py:37-357`, `vendor/zscaler-sdk-python/zscaler/aiguard/policy_match_rules.py:37-338`, `vendor/zscaler-sdk-python/zscaler/aiguard/llm_providers.py:37-457`, `vendor/zscaler-sdk-python/zscaler/aiguard/llm_provider_credentials.py:37-362`, `vendor/zscaler-sdk-python/zscaler/aiguard/llm_applications.py:37-363`, and `vendor/zscaler-sdk-python/zscaler/aiguard/llm_application_credentials.py:37-412`). What remains open is the eight-operation Python-to-retained-contract gap tracked in `ai-security-08`, current AI Guard publication status, and Go SDK, Terraform, MCP, Postman, and Automation Hub wrapper availability.

**Status**: partially resolved — last updated 2026-08-12
**Resolves with**: restored/current vendor publication or vendor confirmation of the AI Guard admin-plane lifecycle, plus SDK/provider/MCP/Postman source exposing operations or confirmation of wrapper roadmap / intended raw-API-only status
...
The retained last-known Automate admin-plane provider-type discovery contract exposes 23 identifiers (`vendor/zscaler-api-specs/automate-zscaler/aiguard-api-reference.json:7503-7533`); AI Guard is absent from the current public route table (`vendor/zscaler-api-specs/automate-zscaler/openapi-validation-report.md:10`, `:156-160`). The 2026 Help chronology separately names User-mode or application support for GitHub Copilot, ElevenLabs, Windsurf, Mistral Vibe, Gamma, and Builder.io, none of which appears under those exact identifiers in the retained enum (`vendor/zscaler-help/ai-guard-release-upgrade-summary-2026.md:14-50`). Static sources do not establish whether those labels are built-in/non-creatable providers, application-level aliases, or contract/Help timing drift.
...
The Automate contract places SIM-search `page`, `size`, `sortBy`, and `sortDir` in query parameters (`vendor/zscaler-api-specs/automate-zscaler/openapi/zcell.openapi.json:11421-11470`). MCP exposes page/size but puts them into the POST body and exposes no sorting; the SDK likewise forwards those kwargs as body fields (`vendor/zscaler-mcp-server/src/zscaler_mcp/tools/zcell/sim_handling.py:125-162`; `vendor/zscaler-sdk-python/zscaler/zcell/sim_handling.py:286-315`). Confirm whether the backend accepts body pagination or silently ignores it.
