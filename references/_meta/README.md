...
- [`../../vendor/zscaler-api-specs/automate-zscaler/`](../../vendor/zscaler-api-specs/automate-zscaler/) — the captured vendor API **contract** (per-product `*-api-reference.json`, field-level `required`/`readonly`/`enum`), cross-surface **divergences** (`*-divergences.md`), the **rosetta** cross-surface field map (`rosetta.md`), and the **issue-routing** worklist (`issue-routing.md`). Reference-grade reconciliation data: search it like a reference even though it lives under `vendor/` — see the carve-out in [`../../agents/loading-discipline.md`](../../agents/loading-discipline.md), and the source-precedence for citing it in [`verification-protocol.md`](./verification-protocol.md).
