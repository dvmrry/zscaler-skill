...
- **Tier 3 — Reasoning content, no SDK/TF write API.** No public SDK/TF write surface (portal-only configuration unless otherwise noted). Reasoning docs exist under `references/<product>/`. Some products now have MCP-only read/inventory adapters; those do not by themselves prove public write APIs. Answer conceptual questions at confidence: medium with the relevant caveat. Do NOT fabricate API specifics.
- **Tier 4 — Paragraph-only awareness.** No SDK and no dedicated reasoning content. One-paragraph treatment in this map. Some products may have MCP-only read adapters; use [`../shared/mcp-runtime.md`](../shared/mcp-runtime.md) for that boundary. Skill recognizes + describes briefly; redirects to TAM / help.
...
## Tier 3 — Reasoning content, no SDK/TF write API

No public SDK / TF write surface (portal-only configuration unless otherwise noted). Help-portal + marketing material has been synthesized into reasoning docs under `references/<product>/`. Skill answers conceptual questions with confidence: medium and a clear "no public SDK/TF write surface" caveat. MCP-only read/inventory adapters are noted per product but do not, by themselves, promote a product into the SDK/TF-backed tiers. **Promote to Tier 1 if Zscaler ships an SDK module** and reference coverage is multi-component; promote to Tier 2 if an SDK lands but reference coverage stays thin.
...
#### Business Insights
**Not a security product** — a Zscaler analytics product that uses ZIA's network visibility to power two business-leadership use cases: (1) **SaaS Application Management** — license right-sizing, shadow IT discovery, spend optimization, portfolio-overlap detection across the licensed app footprint; (2) **Workplace and Workforce Management** — hour-by-hour office utilization for hybrid-work / RTO / capacity planning. Audience is IT leadership, finance, HR, and facilities teams in addition to security admins. Ingests from four sources: ZIA traffic analysis (discovers app usage; can see 30,000+ apps), IdP login data (Okta / Entra ID), direct SaaS app connectors (Okta, M365, Salesforce, ServiceNow, Slack, Box, Google Workspace, GitHub), and admin-configured custom application signatures. **Prerequisites**: ZIA required for SaaS analytics; ZIA + ZCC required for workplace utilization (ZCC location signal is what determines physical presence). Read-only with respect to ZIA — does not write policies. Distinct from ZIA reporting/analytics (own portal, own RBAC, own SKU) and from "Executive Insights" (a different surface within Experience Center). Reasoning doc: [`../business-insights/overview.md`](../business-insights/overview.md). Capture: `vendor/zscaler-help/bi-what-zscaler-business-insights.md`. No public API documented — treat as portal-only.
...
The Tier 3 vs Tier 4 distinction matters: Tier 3 products have substantial reasoning content but can't be configured programmatically, so answers must stay descriptive and not drift into fabricated API specifics. Tier 4 products only have the paragraph above — no synthesized reasoning content — so answers should stay at paragraph depth and route outward.
