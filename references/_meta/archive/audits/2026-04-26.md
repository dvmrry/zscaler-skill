...
type: audit
date: 2026-04-26
scope: "vendor/zscaler-help/ — all 314 raw files + 16 in automate-zscaler/ subdir"
status: archived
schema-version: v0
---

> **Archived audit — pre-schema baseline.** This is the first coverage audit and predates the structured audit template at `references/_meta/archive/audits/_template.md`. Future audits should follow that schema for diff-ability. Most of the high-priority uncovered docs identified here have since been filled (commits `f43ee6c`, `d97c041`, `0a8275c`, `309bd9c`).
...
| `about-saas-security-report.md` | SaaS Security Report (formerly Shadow IT / ZINS) — structure and UI of the report | Partially mentioned in `portfolio-map.md` but no operational reference doc; the GraphQL API endpoint is not documented in references | Extend `references/zia/sdk.md` or new `references/zia/saas-security-report.md` |
...
| `zsdk-understanding-zsdk-error-codes.md` | ZSDK mobile SDK error codes | ZSDK is Tier 2a in `portfolio-map.md` with captures; error codes are high-value for app developers integrating ZSDK; not in any reference | New: `references/zidentity/zsdk-error-codes.md` or extend ZSDK capture section in `portfolio-map.md` |
| `zsdk-ranges-limitations.md` | ZSDK limits — max tunnels, token lifetimes, supported OS versions | Important constraint reference for developers building ZSDK apps; no reference doc | Extend ZSDK section in `portfolio-map.md` or new ZSDK reference file |
| `zsdk-understanding-app-connector-throughput.md` | ZSDK App Connector throughput sizing and performance | Capacity planning for ZSDK deployments; not covered anywhere | Extend `portfolio-map.md` ZSDK section or new ZSDK reference |
...
**Gap assessment:** AI Security has solid overview coverage. Three potential gaps: (1) AI Red Teaming is mentioned in `portfolio-map.md` but no vendor doc for it exists in the corpus — this is a corpus gap, not a coverage gap; (2) The DaaS deployment mode for AI Guard has only brief mention; (3) Integration with ZIA GenAI URL categories and DLP prompt scanning is partially but not fully treated. No new reference file urgently needed; extend `references/ai-security/overview.md` with deployment mode depth.
...
| `about-zpa-app-connectors-deception.md` | **Covered** | ZPA connector + Deception integration noted in overview and `clarifications.md` |
...
| Vendor filename | Coverage state | Notes |
|---|---|---|
| `microsegmentation-marketing.md` | **Covered** | Synthesized into `references/zms/overview.md`; cited in `portfolio-map.md` |
...
| `zpa-about-isolation-policy.md` | **Covered** | ZPA isolation policy structure referenced in `portfolio-map.md` and `references/zbi/policy-integration.md` |
...
|---|---|---|---|
| NSS architecture | `about-nss-feeds.md`, `about-nss-servers.md`, `about-cloud-nss-feeds.md`, `about-log-streaming-service.md`, `understanding-nanolog-streaming-service.md`, `General_Guidelines_for_NSS_Feeds_and_Feed_Formats.txt` | **Covered** | `references/shared/nss-architecture.md` treats this thoroughly; CSV schema files are cited in log-schema references |
...
| Business continuity | `understanding-business-continuity-cloud-components.md` | **Covered** | Treated in `portfolio-map.md` and `references/shared/cloud-architecture.md` |
...
**`references/zia/ssl-inspection.md`** covers: `About_SSL_TLS_Inspection_Policy.txt`, `Best_Practices_for_Testing_and_Rolling_Out_SSL_TLS_Inspection.txt`, `SSL_Inspection_Deployment_and_Operations_Guide.txt`, `ZIA_SSL_Inspection_Leading_Practices_Guide.txt`, `configuring-ssl-tls-inspection-policy.md`, `configuring-ssl-inspection-zscaler-client-connector.md`

**`references/zia/url-filtering.md`** covers: `About_URL_Categories.txt`, `Configuring_the_URL_Filtering_Policy.txt`, `URL_Filtering_Deployment_and_Operations_Guide.txt`, `Configuring_URL_Categories_Using_API.txt`, `Recommended_URL_&_Cloud_App_Control_Policy.txt`
...
**`references/shared/nss-architecture.md`** covers: `about-nss-feeds.md`, `about-nss-servers.md`, `about-cloud-nss-feeds.md`, `about-log-streaming-service.md`, `understanding-nanolog-streaming-service.md`, `General_Guidelines_for_NSS_Feeds_and_Feed_Formats.txt`
...
**`references/zpa/app-connector.md`** covers: `about-app-connectors.md`, `about-connector-provisioning-keys.md`, `understanding-connector-software-updates.md`, `zpa-about-connector-groups.md`, `Access_Policy_Deployment_and_Operations_Guide.txt`, `Understanding_App_Connector_Metrics_Log_Fields.txt`
...
**`references/zpa/policy-precedence.md`** covers: `About_Policies.txt`, `Understanding_Policy_Enforcement.txt`, `About_Access_Policy.txt`, `Access_Policy_Configuration_Examples.txt`, `Configuring_Access_Policies.txt`, `Configuring_Advanced_Policy_Settings.txt`, `Configuring_Defined_Application_Segments.txt`, `Using_Application_Segment_Multimatch.txt`, `Understanding_Application_Access.txt`

**`references/zpa/app-segments.md`** covers: `about-segment-groups.md` (partial), `zpa-user-to-app-segmentation-refarch.txt`
...
**`references/zpa/log-receivers.md`** covers: `Understanding_the_Log_Stream_Content_Format.txt`, `Understanding_User_Activity_Log_Fields.txt`, `Understanding_User_Status_Log_Fields.txt`
...
**`references/shared/log-correlation.md`** covers: `Understanding_User_Activity_Log_Fields.txt`, `Understanding_User_Status_Log_Fields.txt` (partial)
...
|---|---|
| `agentic-secops-security-operations-marketing.md` | Marketing — synthesized into `portfolio-map.md`; no new operational content |
| `asset-exposure-management-caasm-marketing.md` | Marketing — synthesized into `portfolio-map.md` |
| `data-fabric-for-security-marketing.md` | Marketing — synthesized into `portfolio-map.md` |
| `dspm-marketing.md` | Marketing — synthesized into `portfolio-map.md` |
| `easm-introducing-marketing.md` | Marketing intro — `easm-what-is-zscaler-easm.md` cited in `portfolio-map.md`; this is duplicative |
| `easm-what-is-zscaler-easm.md` | Awareness-level marketing — covered in `portfolio-map.md` |
| `itdr-zscaler-identity-protection-marketing.md` | Marketing — synthesized into `portfolio-map.md` |
| `microsoft-copilot-security-marketing.md` | Marketing — synthesized into `portfolio-map.md` |
| `microsegmentation-marketing.md` | Marketing — synthesized into `portfolio-map.md` and `references/zms/overview.md` |
| `risk360-product-marketing.md` | Marketing — synthesized into `references/risk360/overview.md` |
| `security-operations-suite-marketing.md` | Marketing umbrella — synthesized into `portfolio-map.md` |
| `shadow-it-marketing.md` | Marketing — `shadow-it-saas-security-report-zia.md` is the operational doc |
| `uvm-unified-vulnerability-management-marketing.md` | Marketing — synthesized into `portfolio-map.md` |
| `zero-trust-exchange-zte-marketing.md` | Marketing — synthesized into `portfolio-map.md` |
| `zero-trust-microsegmentation-marketing.md` | Marketing — synthesized into `portfolio-map.md` and `references/zms/overview.md` |
| `zscaler-b2b-marketing.md` | Marketing — synthesized into `portfolio-map.md` |
| `zscaler-cellular-marketing.md` | Marketing — synthesized into `portfolio-map.md` |
| `zscaler-govcloud-innovations.md` | Marketing / federal cloud innovations highlight reel — synthesized into `portfolio-map.md` |
| `zscaler-government-public-sector-marketing.md` | Marketing — synthesized into `portfolio-map.md` |
| `zscaler-resilience-marketing.md` | Marketing — synthesized into `portfolio-map.md` |
...
| `shadow-it-saas-security-report-zia.md` | Partially operational but primarily a report walkthrough; `portfolio-map.md` covers it adequately for Tier 2 awareness; upgrade to reference if ZINS/GraphQL API coverage is deepened |
...
| `zsdk-what-zscaler-sdk-mobile-apps.md` | ZSDK product intro — covered in `portfolio-map.md` ZSDK section |
