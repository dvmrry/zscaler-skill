...
| `scripts/policy_simulator.py` | functional (module, not a CLI) | Importable Python module: pure-function ZIA URL filter evaluator. `simulate_url_filter(request, rules, categories)` returns a `SimulationResult` with the matched rule, action, resolved category, and full per-rule evaluation trace. `diff_simulations(before, after)` for change validation. Models rule order, disabled-rule-holds-slot, leading-period wildcard category resolution, and basic criteria filtering. Documented gaps (CAC, DLP, two-pass SSL, full specificity-wins) in `references/_policy-simulation.md`. |
