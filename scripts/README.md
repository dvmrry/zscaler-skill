...
| **Reasoning helpers** | `agent_patterns.py` (lib), `ab-test-prompt.py` (experimental placeholder), `investigator-artifacts.mjs` (exports `renderCaseReport` — artifact-derived report, no free narrative), `investigator-mcp-server.mjs` (MCP stdio transport for the helper gates; registered in `.mcp.json`; exposes resources `investigator://case/{slug}/report\|journal\|status` and prompts `investigate`/`resume-case`), `investigator-mcp-server.test.mjs` (node:test suite for the MCP server), `check-mcp-conformance.mjs` (in-process JSON-RPC conformance gate; wired into `check-fast.mjs`; degrades gracefully if official inspector unavailable) |
| **Bridge harness (LOCAL-ONLY)** | `bridge/run-investigation.mjs` — drives the `devin` CLI through a multi-turn scripted investigation and independently verifies the case's on-disk gate state via this repo's own helper exports (`caseStatus`/`renderCaseReport`); **needs `devin` auth + network, not run in CI** and deliberately not wired into `check-fast.mjs`. Scenarios under `bridge/scenarios/`; pure-logic fixtures in `bridge/run-investigation.test.mjs` (no `devin` spawn). See `bridge/README.md`. |
